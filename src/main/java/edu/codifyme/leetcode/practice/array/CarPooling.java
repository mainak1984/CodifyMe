...
/**
 * 1094. Car Pooling
 * MEDIUM: https://leetcode.com/problems/car-pooling/
 *
 ...
 *
 * Approach:
 * Go over the range of pickup and drop points and estimate capacity to check if anything is shooting higher.
 * We can use a TreeMap to track or use an array to do a bucket sort
 */
public class CarPooling {
    public boolean carPooling(int[][] trips, int capacity) {
        ...
        for (int[] row: trips) {
            map.put(row[1], map.getOrDefault(row[1], 0) + row[0]);
            map.put(row[2], map.getOrDefault(row[2], 0) - row[0]);
        }

        int runningCapacity = 0;

        for (Integer key: map.keySet()) {
            runningCapacity += map.get(key);
