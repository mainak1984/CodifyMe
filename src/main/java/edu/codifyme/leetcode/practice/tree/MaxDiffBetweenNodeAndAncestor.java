 ...
 *
 * Approach:
 * Since the problem asks us the Maximum Difference, maybe we do not need to compare all ancestor for a given node and
 * we only need to compare the ancestors with Maximum value and Minimum value.
 * Therefore, for a given node, we only need the maximum value and the minimum value from the root to this node.
 * To achieve this, we can define a function helper to start recursion, which receives a node and two integers, the
