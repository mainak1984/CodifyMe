...
public class MinimumCostForTickets {
    public int mincostTickets(int[] days, int[] costs) {
        int[] dayConfig = new int[]{1, 7, 30};
        int[] dp = new int[days.length+1];

        Arrays.fill(dp, Integer.MAX_VALUE);
        dp[0] = 0;

        TreeMap<Integer, Integer> daysMap = new TreeMap<Integer, Integer>();
        daysMap.put(0,0);
        ...
        for (int i = 1; i < days.length+1; i++) {
            for (int j = 1; j < costs.length+1; j++) {
                Integer targetDayKey = daysMap.floorKey(days[i-1] - dayConfig[j-1]);
                if (targetDayKey == null) {
                    dp[i] = Math.min(dp[0] + costs[j-1], dp[i]);
                } else if (targetDayKey == days[i-1]) {
                    dp[i] = Math.min(dp[i-1] + costs[j-1], dp[i]);
                } else {
                    int targetDayIndex = daysMap.get(targetDayKey);
                    dp[i] = Math.min(dp[targetDayIndex] + costs[j-1], dp[i]);
                }
            }
        }
