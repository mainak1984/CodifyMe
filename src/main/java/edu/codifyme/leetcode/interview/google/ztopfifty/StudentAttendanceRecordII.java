 ...
 * Before introducing the way to calculate the number of all possible attendance records with length n, we divide the
 ...
 * It can be inferred that
 *
 * If we add a 'P' to an attendance records with length n - 1, we will get an attendance records ended with 'P' with
 * length n.
 ...
 * Similarly,
 *
 * If we add a 'L' to an attendance records with length n - 1, we will get an attendance records ended with 'L' with
 * length n.
 ...
 * We need to consider the situations when we can add 'L' to an attendance record with length n - 1 and it's still
 ...
 * Similarly,
 *
 * If we add a 'A' to an attendance records with length n - 1, we will get an attendance records ended with 'A' with
 * length n.
 * But the resulting attendance records must be regarded as rewardable!
 *
 * As the rule is that a record is regarded as rewardable if it doesn't contain
 ...
 * We need to consider the situations when we can add 'A' to an attendance record with length n - 1 and it's still
 * regarded as rewardable.
 *
 * For an attendance record with length n - 1,
 *
 ...
 * In 2.2.3, 2 new series noAP(n) and noAL(n) is introduced. Now, we focus on the recursive relation in noAP(n) and
 * noAL(n).
 *
 * For noAP(n), we need to consider the situations when we can add 'P' to an attendance record with length n - 1 and
 * no 'A' and it's still regarded as rewardable.
 *
 ...
 *
 * noAP(1) = noAL(1) = 1.
 * For noAL(n), we need to consider the situations when we can add 'L' to an attendance record with length n - 1 and
 * no 'A' and it's still regarded as rewardable.
 *
 ...
 *
 * Since the result is generated by adding a lot of middle results together, in order to make sure that every middle
 * result and the final result won't exceed INT_MAX, we need to do mod for every middle result, and for every
